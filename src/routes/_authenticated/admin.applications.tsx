...
const COLUMNS = ["id", "job_id", "user_id", "status", "applied_at"] as const;
...
function Page() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("applications").select("id, job_id, user_id, status, applied_at").limit(500);
